...
'''
redmart redis problem monitoring script
'''
...
class RedisStats:
    ...
    def __init__(self, port='6379', passwd=None, host='10.0.0.2'):
...
def main():
    ...
    for inst in insts_list:
        port = commands.getoutput(
            "sed -n 's/^port *\([0-9]\{4,5\}\)/\\1/p' %s" % inst)
        passwd = commands.getoutput(
            "sed -n 's/^requirepass *\([^ ]*\)/\\1/p' %s" % inst)
        metric = "redis"
        endpoint = ip
        tags = 'port=%s' % port
        ...
        for key, vtype in monit_keys:
            if key == 'keyspace_hit_ratio':
                try:
                    value = float(stats[
                                  'keyspace_hits']) / (int(stats['keyspace_hits']) + int(stats['keyspace_misses']))
    ...
    url = 'http://10.0.0.1:1988/v1/push'
...
if __name__ == '__main__':
    proc = commands.getoutput(
        ' ps -ef|grep %s|grep -v grep|wc -l ' % os.path.basename(sys.argv[0]))
    if int(proc) < 5:
        main()
